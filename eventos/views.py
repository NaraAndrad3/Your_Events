...
from .models import Events, Certificado
...
def gerenciar_evento(request):
    if request.method == "GET":
        ...
        eventos = Events.objects.filter(creator=request.user)
        if nome:
...
def participantes_evento(request,id):
    evento = get_object_or_404(Events, id=id)
    if not evento.creator == request.user:
...
def gerar_certificado(request, id):
    ...
    path_template = os.path.join(settings.BASE_DIR, 'templates/static/events/img/template_certificado.png')
